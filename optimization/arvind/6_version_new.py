...
targets = [
    {"structure": [1, 0, 2, 3, 0, 4, 5, 0, 6], "desired_yield": 0.9},
]
...
tot_num_structures = n_1 + n_2 + n_3 
...
patchy_vals = jnp.full(2, 6.0)  # FIXME for optimization over specific attraction strengths
#patchy_vals = jnp.full(n_morse_vals, 4.0) 
# Generate initial concentrations dynamically based on the number of monomers
initial_concentrations = jnp.full(num_monomers, 0.00015)
...
def get_log_z_all(opt_params):
    ...
    log_z_all = jnp.concatenate([log_z_1, log_z_2, log_z_3]) 
...
for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
    ...
    if key1 in data and key2 in data and key3 in data:
        monomer_counts.append(jnp.concatenate([data[key1], data[key2], data[key3]]))
