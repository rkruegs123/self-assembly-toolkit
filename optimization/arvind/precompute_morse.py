...
patchy_vals = jnp.array([7.5, 5.0, 500.0,  2.5]) # default_strong_eps, morse_narrow_alpha, A_rep, rep_strong_alpha
# Energy helper functions
rep_rmax_table = jnp.full((n_species, n_species), 2*vertex_radius)

def make_tables(patchy_params):
    small_value = 1e-12
    morse_eps_table = jnp.full((n_species, n_species), small_value)
    default_strong_eps = patchy_params[0]
    morse_eps_table = morse_eps_table.at[jnp.array([2, 3]), jnp.array([3, 2])].set(default_strong_eps)
    morse_eps_table = morse_eps_table.at[jnp.array([4, 5]), jnp.array([5, 4])].set(default_strong_eps)

    morse_narrow_alpha = patchy_params[1]
    ...
    rep_A_table = rep_A_table.at[vertex_species, vertex_species].set(patchy_params[2])

    rep_strong_alpha = patchy_params[3]
    rep_alpha_table = jnp.full((n_species, n_species), rep_strong_alpha)
    
    return morse_eps_table, morse_alpha_table, rep_A_table, rep_alpha_table

def pairwise_morse(ipos, jpos, i_species, j_species, patchy_params):
    morse_eps_table, morse_alpha_table, _, _ = make_tables(patchy_params)
...
def pairwise_repulsion(ipos, jpos, i_species, j_species, patchy_params):
    _, _, rep_A_table, rep_alpha_table = make_tables(patchy_params)
...
@jit
def dimer_energy(q, pos, species, patchy_params=patchy_vals):
    positions = get_positions(q, pos)  # 18 particles (9 per monomer)
    pos1 = positions[:9]  # First monomer
    pos2 = positions[9:]  # Last monomer
    species1 = species[:3]
    species2 = species[3:]
    species1 = jnp.repeat(species1, 3)
    species2 = jnp.repeat(species2, 3)
    tot_energy = jnp.sum(morse_func(pos1, pos2, species1, species2, patchy_params))
    tot_energy += jnp.sum(rep_func(pos1, pos2, species1, species2, patchy_params))
    return tot_energy 

@jit
def trimer_energy(q, pos, species, patchy_params=patchy_vals):
    ...
    species1 = jnp.repeat(species1, 3)
    species2 = jnp.repeat(species2, 3)
    species3 = jnp.repeat(species3, 3)
    tot_energy = jnp.sum(morse_func(pos1, pos2, species1, species2, patchy_params))
    tot_energy += jnp.sum(morse_func(pos1, pos3, species1, species3, patchy_params))
    tot_energy += jnp.sum(morse_func(pos2, pos3, species2, species3, patchy_params))
    tot_energy += jnp.sum(rep_func(pos1, pos2, species1, species2, patchy_params))
    tot_energy += jnp.sum(rep_func(pos1, pos3, species1, species3, patchy_params))
    tot_energy += jnp.sum(rep_func(pos2, pos3, species2, species3, patchy_params))
    return tot_energy

# Functions for computing partition functions
def hess(energy_fn, q, pos, species, patchy_params=patchy_vals):
    H = hessian(energy_fn)(q, pos, species)
    evals, evecs = jnp.linalg.eigh(H)
    return evals, evecs

def compute_zvib(energy_fn, q, pos, species, patchy_params):
    evals, evecs = hess(energy_fn, q, pos, species, patchy_params)
...
zrot_mod_sigma_dim = compute_zrot_mod_sigma(
    ...
    jnp.array([1,0,2,3,0,4]), seed=0,
    nrandom=int(1e5))

zrot_mod_sigma_trim = compute_zrot_mod_sigma(
    trimer_energy, trimer_rb, trimer_shape,
    jnp.array([1,0,2,3,0,4,5,0,6]), seed=0,
    nrandom=int(1e5))

dimer_pc_species = data['dimer_pc_species']
trimer_pc_species = data['trimer_pc_species']

def get_log_z_all(patchy_params):
    def compute_log_z_dimer(dimer_species, dimer_sigma):
        zvib_dim = compute_zvib(dimer_energy, dimer_rb, dimer_shape, dimer_species, patchy_params)
        e0 = dimer_energy(dimer_rb, dimer_shape, dimer_species, patchy_params)
    ...
    def compute_log_z_trimer(trimer_species, trimer_sigma):
        zvib_trim = compute_zvib(trimer_energy, trimer_rb, trimer_shape, trimer_species, patchy_params)
        e0 = trimer_energy(trimer_rb, trimer_shape, trimer_species, patchy_params)
        boltzmann_weight = jnp.exp(-e0 / kT)
...
m_conc = jnp.array([conc, conc, conc])
...
def optimality_fn(log_concs_struc, log_z_list):
    return grad(lambda log_concs_struc, log_z_list : loss_fn(log_concs_struc, log_z_list)[0])(log_concs_struc, log_z_list)

inner_solver_logs = []

@implicit_diff.custom_root(optimality_fn)
def inner_solver(init_guess,log_z_list):
    gd = GradientDescent(fun=lambda log_concs_struc, log_z_list : loss_fn(log_concs_struc, log_z_list)[0], maxiter=6000, implicit_diff=True)
    sol = gd.run(init_guess, log_z_list=log_z_list)
    
    final_params = sol.params
    final_loss, combined_losses, loss_var = loss_fn(final_params, log_z_list)
    ...
    #print("Max value :", max_loss)
    #print("Second max value :", second_max_loss)
     #print("Combined losses:", combined_losses)
    
    return final_params

def ofer(patchy_params):
    
    log_z_list = get_log_z_all(patchy_params)
    ...
    return target_yield, offtarget_dim , offtarget_tri, yield_sum

def ofer_grad_fn(patchy_params):
    target_yield = ofer(patchy_params)[0]
    return -target_yield

def project(param):
    return jnp.clip(param, a_min=1e-4)

our_grad_fn = jit(value_and_grad(ofer_grad_fn))
...
with open("morse_log.txt", "w") as log_file:
    log_file.write("Iteration\tTarget_Yield\tOfftarget_Dim\tOfftarget_Tri\tTot_yield\tEps")
    ...
    for i in tqdm(range(n_outer_iters)):
        ...
        target_yield, offtarget_dim, offtarget_tri, tot_yield = ofer(params)

        log_file.write(f"{i+1}\t{target_yield}\t{offtarget_dim}\t{offtarget_tri}\t{tot_yield }\t{params}\n")
