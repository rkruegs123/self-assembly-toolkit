...
monomer_counts = {k: v for k, v in data.items() if 'counts' in k}
monomers = sorted(set(k.split('_')[0] for k in monomer_counts.keys()))

nper_structure = []
for monomer in monomers:
    counts = []
    for count_type in ['mon_counts', 'dimer_counts', 'trimer_counts']:
        counts.append(data[f'{monomer}_{count_type}'])
    nper_structure.append(jnp.concatenate(counts))

nper_structure = jnp.array(nper_structure)
