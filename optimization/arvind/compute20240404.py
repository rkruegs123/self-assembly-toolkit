...
# TODO: make monomer energy function that jives well with things
...
if test_energy:
    ...
    trimer_energy(trimer_rb, trimer_shape, data['trimer_pc_species'][1])

# TODO: add example calls for monomer energy function
...
def compute_zvib(energy_fn, q, pos, species):
    """
    energy_fn: A callable of the form energy_fn(q, pos, species)
    q: is the center of mass positions
    pos: body-frame positions of the rigid body
    species: the species of the unwrapped positions
    """
...
test_pf = False
if test_pf:
    compute_zvib(dimer_energy, dimer_rb, dimer_shape, data['dimer_pc_species'][1])
    trial_nsamples = [10, 100, 1000, 10000, 100000, 250000, 500000]
    for nsamples in trial_nsamples:
        start = time.time()
        trial_zrot = compute_zrot_mod_sigma(
            trimer_energy, trimer_rb, trimer_shape,
            data['trimer_pc_species'][1], seed=0,
            nrandom=nsamples)
        end = time.time()
        print(f"# Samples={nsamples}, Zrot={onp.round(trial_zrot, 4)}, time={onp.round(end - start, 2)} seconds")


# TODO: Precompute partition functions and save to file (or load from file)
sigmas = [1, 1, 1] # FIXME: think about these. Monomer, dimer, trimer.
mon_sigma = data['mon_sigma']
...
z_mon = compute_zc(boltzmann_weight, zrot_mod_sigma_mon, zvib_mon, sigmas[0], V)
z_mons = jnp.full(len(data['mon_pc_species']), z_mon)

## Precompute the dimer partition functions. Note: we only compute Zrot once
zrot_mod_sigma_dim = compute_zrot_mod_sigma(
    dimer_energy, dimer_rb, dimer_shape,
    data['dimer_pc_species'][0], seed=0,
    nrandom=int(1e5))
z_dims = list()
for dimer_species in tqdm(data['dimer_pc_species'], desc="Computing dimer partition functions"):
    ...
    z_dims.append(z_dim)
print(onp.shape(z_dims))
z_dims_a = jnp.array(z_dims)
log_z_dims = jnp.array(z_dims_a)

print(onp.shape(log_z_dims))
