...
class MorsePotential():
    ...
    def dE(r, rmin, rmax, D0, alpha, r0):
        if r >= rmax:
            return 0.
        return D0*(-2*alpha*jnp.exp(-2*alpha*(r-r0)) + 2*alpha*jnp.exp(-alpha*(r-r0)))

    @staticmethod
    def d2E(r, rmin, rmax, D0, alpha, r0):
        if r >= rmax:
                return 0.
        return D0*(4*alpha**2*jnp.exp(-2*alpha*(r-r0)) - 2*alpha**2*jnp.exp(-alpha*(r-r0)))

    @staticmethod
    def E_dE(r, rmin, rmax, D0, alpha, r0):
        if r >= rmax:
            return (0.,0.)
        E = D0*(jnp.exp(-2*alpha*(r-r0)) - 2*jnp.exp(-alpha*(r-r0)))
        g = D0*(-2*alpha*jnp.exp(-2*alpha*(r-r0)) + 2*alpha*jnp.exp(-alpha*(r-r0)))
        return (E,g)
    ...
    def E_dE_d2E(r, rmin, rmax, D0, alpha, r0):
        ...
        E = D0*(jnp.exp(-2*alpha*(r-r0)) - 2*jnp.exp(-alpha*(r-r0)))
        g = D0*(-2*alpha*jnp.exp(-2*alpha*(r-r0)) + 2*alpha*jnp.exp(-alpha*(r-r0)))
        k = D0*(4*alpha**2*jnp.exp(-2*alpha*(r-r0)) - 2*alpha**2*jnp.exp(-alpha*(r-r0)))
        return (E,g,k)
...
class MorseXPotential(MorsePotential):
    ...
    def dE(r, rmin, rmax, D0, alpha, r0, ron):
            return (MorsePotential().dE(r, rmin, rmax, D0, alpha, r0) \
                    * Potential_S(r, ron, rmax) \
                    + MorsePotential().E(r, rmin, rmax, D0, alpha, r0) \
                    * Potential_dS(r, ron, rmax))

    @staticmethod
    def d2E(r, rmin, rmax, D0, alpha, r0, ron):
        return (MorsePotential().d2E(r, rmin, rmax, D0, alpha, r0) \
                * Potential_S(r, ron, rmax) \
                + 2 * MorsePotential().dE(r, rmin, rmax, D0, alpha, r0) \
                * Potential_dS(r, ron, rmax) \
                + MorsePotential().E(r, rmin, rmax, D0, alpha, r0) \
                * Potential_d2S(r, ron, rmax))
